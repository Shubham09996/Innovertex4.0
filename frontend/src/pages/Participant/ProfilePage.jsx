...
const ProfilePage = () => {
  ...
  const [badges] = useState([
    ...
    { id: 2, name: "Innovator", icon: "💡", desc: "Submitted an innovative idea" },
    { id: 3, name: "Leader", icon: "👑", desc: "Led a winning team" },
    { id: 4, name: "Code Master", icon: "💻", desc: "Top coder in challenge" },
    ...
    <main className="min-h-screen bg-bg text-text py-24 px-6">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {activeTab === "Activity" && (
            <div className="space-y-6">
              ...
              >
                ...
                <p className="mt-4 text-sm text-muted">Consistent streaks earn bonus points & badges.</p>
              </motion.div>
              ...
              >
                ...
                <ul className="space-y-3">
                  <ActivityItem title="Won 1st place — AI Hack" time="2 days ago" accent="from-pink-500 to-purple-500" />
                  <ActivityItem title="Published Project: SmartVision" time="7 days ago" accent="from-indigo-500 to-blue-400" />
                  <ActivityItem title="Mentored Session: React Basics" time="12 days ago" accent="from-green-400 to-teal-300" />
                </ul>
              </motion.div>
            </div>
          )}
        ...
        <aside className="space-y-6">
          ...
          >
            <h4 className="text-sm text-muted mb-4">Global Standing</h4>
            <CircularRank percent={78} rank="#123" points={stats.points} />
            <div className="mt-6 space-y-3 text-left">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted">Challenge Score</span>
                <span className="font-semibold text-text">91</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted">Mentorship</span>
                <span className="font-semibold text-text">5 sessions</span>
              </div>
            </div>
          </motion.div>
...
const CircularRank = ({ percent = 70, rank = "#123", points = 1500 }) => {
...
const GitHubLikeActivityGraph = () => {
  ...
  const renderGitHubGraph = () => {
    ...
    weeks.forEach((week, weekIndex) => {
      const firstActualDay = week.find(day => day && day.date !== null);
      if (firstActualDay && firstActualDay.date.getMonth() !== currentMonth) {
